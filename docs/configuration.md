...
| `autoDiscoverModels` | `boolean` | provider default | Fetch `{baseUrl}/v1/models` at startup and merge the result into the catalog. On by default for gateway-style providers (`ai-gateway`, `openrouter`, `requesty`, `omniroute`). See [Model discovery](#model-discovery) below. |
