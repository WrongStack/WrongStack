...
function startRpcServer(
  handler: (method: string, params: Record<string, unknown>, id: number | null) => string | string[],
): TrackedServer {
  const path =
...
function healthFetch(socketPath: string, responder?: (url: string) => unknown): typeof fetch {
  return (async (url: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (target.endsWith("/api/health")) {
      const body = { status: "ok", socket_path: socketPath } satisfies WrongTraceHealth;
...
describe("createIpcTransport (JSON-RPC 2.0, \\n-framed)", () => {
  ...
  it("round-trips a JSON-RPC result envelope", async () => {
    ...
    const res = await ipc.call<{ health_score: number }>("telemetry/file_health", { file_path: "a.ts" });
    expect(res.result).toMatchObject({ method: "telemetry/file_health", file_path: "a.ts", health_score: 100 });
  ...
  it("surfaces daemon error envelopes as { result: null, error } — never as a fake result", async () => {
    const server = startRpcServer((method) =>
      JSON.stringify({ jsonrpc: "2.0", id: 1, error: { code: -32601, message: `method not found: ${method}` } }),
    );
    servers.push(server);
    const ipc = createIpcTransport(server.path);
...
describe("createWrongTraceClient() IPC-first routing", () => {
  ...
  it("getFileHealth prefers the daemon pipe and skips HTTP", async () => {
    ...
    expect(health).toMatchObject({ file_path: "internal/server/server.go", health_score: 35, is_fragile: true });
  ...
  it("getFileHealth falls back to HTTP when the pipe answers with an error envelope", async () => {
    ...
    globalThis.fetch = healthFetch(server.path, (url) => {
      if (url.includes("/api/file/health")) {
        return {
          file_path: "x.ts", health_score: 80, is_fragile: false, recent_thrashing_count: 0, is_locked: false,
        };
      }
      return undefined;
    });
