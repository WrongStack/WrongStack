...
function offlineStub(): WrongTraceClient {
  return {
    ...
    async getFileHealth() { return null; },
    async getSymbolLineage() { return []; },
    async getFrictionMatrix() { return []; },
    async getAtlas() { return null; },
    async lockFile() { return null; },
    async unlockFile() { return null; },
    async reportTelemetry() { return null; },
...
function stubWith(opts: {
  ...
  return {
    isAvailable: true,
    async getHealth() { return null; },
    async getFileHealth() { return Promise.resolve(opts.fileHealth as never); },
    async getSymbolLineage() { return []; },
    async getFrictionMatrix() { return Promise.resolve(friction as never); },
    async getAtlas() { return null; },
    async lockFile() { return null; },
    async unlockFile() { return null; },
    async reportTelemetry() { return null; },
...
describe("getCrossAgentRisk", () => {
  ...
  it("flags locked files with risk 100", async () => {
    const wt = stubWith({
      fileHealth: { health_score: 100, is_fragile: false, recent_thrashing_count: 0, is_locked: true, lock_owner: "agent-x", lock_reason: "active refactor" },
    });
  ...
  it("keeps a lock held when lock_expires_at is in the future, surfacing the expiry", async () => {
    ...
    const wt = stubWith({
      fileHealth: { health_score: 100, is_fragile: false, recent_thrashing_count: 0, is_locked: true, lock_owner: "agent-x", lock_expires_at: future },
    });
    const r = await getCrossAgentRisk(wt, "src/foo.ts");
    expect(r.band).toBe("locked");
    expect(r.risk).toBe(100);
  ...
  it("treats an expired lock as stale — falls through to health scoring instead of blocking forever", async () => {
    ...
    const wt = stubWith({
      fileHealth: { health_score: 90, is_fragile: false, recent_thrashing_count: 0, is_locked: true, lock_owner: "crashed-agent", lock_expires_at: past },
      friction: [],
    });
    const r = await getCrossAgentRisk(wt, "src/foo.ts");
  ...
  it("treats a lock with no expiry as held (daemon TTL missing)", async () => {
    const wt = stubWith({
      fileHealth: { health_score: 100, is_fragile: false, recent_thrashing_count: 0, is_locked: true, lock_owner: "agent-x" },
    });
    const r = await getCrossAgentRisk(wt, "src/foo.ts");
    expect(r.band).toBe("locked");
  ...
  it("escalates fragile files to band:fragile", async () => {
    const wt = stubWith({
      fileHealth: { health_score: 25, is_fragile: true, recent_thrashing_count: 1, is_locked: false },
    });
    const r = await getCrossAgentRisk(wt, "src/foo.ts");
  ...
  it("penalizes thrashing above 3 cycles", async () => {
    const wt = stubWith({
      fileHealth: { health_score: 90, is_fragile: false, recent_thrashing_count: 8, is_locked: false },
    });
    const r = await getCrossAgentRisk(wt, "src/foo.ts");
...
describe("summarizeFriction", () => {
  ...
  it("produces a top-pair line for live data shape", () => {
    const liveShape = {
      edges: [
        { author_model: "MiniMax-M3", overwriter_model: "gemini-3.7-flash", conflict_count: 3, is_self_thrash: false },
        { author_model: "glm-5.3", overwriter_model: "glm-5.3", conflict_count: 5, is_self_thrash: true },
      ],
      total_collisions: 8,
    };
    ...
    expect(["MiniMax-M3 ↔ gemini-3.7-flash (3 conflicts)", "glm-5.3 ↔ glm-5.3 (5 conflicts)"])
      .toContain(s.topPair);
...
describe("getRecentActivity", () => {
  ...
  it("filters events by file path and sorts newest-first", async () => {
    const matrix: unknown = {
      recent_collisions: [
        { file_path: "src/foo.ts", overwriter_model: "x", overwriter_time: "2026-08-24T10:00:00Z", action: "MODIFIED" },
        { file_path: "src/other.ts", overwriter_model: "y", overwriter_time: "2026-08-24T11:00:00Z", action: "MODIFIED" },
        { file_path: "src/foo.ts", overwriter_model: "z", overwriter_time: "2026-08-24T12:00:00Z", action: "DELETED" },
      ],
    };
...
describe("digestAtlas", () => {
  ...
  it("counts fragile files and lists thrash hotspots", () => {
    const atlas = {
      workspaces: ["a", "b"],
      packages: [
        { name: "p1", files: [{ health_score: 20, recent_thrashing_count: 0 }, { health_score: 95, recent_thrashing_count: 8 }] },
