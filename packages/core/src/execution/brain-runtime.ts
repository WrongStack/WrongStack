 ...
 *
 * These two sets are the SINGLE source of truth for which fields `mergePatch`
 * accepts. Adding a knob to `BrainConfigPatch` without listing it here makes
 * `apply()` throw `Unknown brain config field`, so keep them in step.
 */
...
const STRUCTURAL_PATCH_KEYS: ReadonlySet<string> = new Set([
  'models',
  'strategy',
  'decisionTimeoutMs',
  'council',
...
const TRACE_CONTENT_MODES: ReadonlySet<string> = new Set(['none', 'redacted', 'full']);
const DENY_TERMINAL_MODES: ReadonlySet<string> = new Set(['never', 'when-decided', 'always']);
const TERMINAL_POLICIES: ReadonlySet<string> = new Set([
  'conservative',
  'deny-all',
