...
describe('domain lifecycle bridge', () => {
  it('captures uncovered domains, scopes identities and redacts prose', async () => {
    ...
    events.emit('memory.candidate_rejected', {
    ...
    expect(recorded[0]).toMatchObject({
      scope: { sessionId: 's' },
      correlation: { traceId: 'memory-trace' },
      resource: { kind: 'other' },
    });
    ...
    expect(recorded[2]).toMatchObject({
      resource: { kind: 'artifact', id: 'worktreeId:wt-1' },
      outcome: 'success',
    });
  ...
  it('truncates recentMail and recentTools arrays to 5 entries', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-trunc-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    ...
    const recentTools = leader['recentTools'] as {
      items: unknown[];
      total: number;
      truncated: true;
    };
  ...
  it('excludes rollup-owned snapshots and network chatter from raw persistence', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-excl-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    const context = createChronicleContext({ installationId: 'i', machineId: 'm' }, 'trace');
    ...
    events.emit('network.request.started', {
      sessionId: 's',
      requestId: 'r1',
      initiator: 'provider',
      operationName: 'op',
    ...
    events.emit('network.request.failed', {
      sessionId: 's',
      requestId: 'r2',
      initiator: 'tool',
      operationName: 'op',
    ...
    events.emit(
      'runtime.health.sampled' as never,
      { sessionId: 's', eventLoop: { utilization: 0.1 } } as never,
    );
  ...
  it('persists rejectedDetail and caps it at 5 entries like activated/injected', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-memrej-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    const context = createChronicleContext({ installationId: 'i', machineId: 'm' }, 'trace');
    ...
    const rejected = Array.from({ length: 20 }, (_, i) => ({
      ...
      reason: `score 0.${50 + i} below floor 0.85`,
      relationStrength: 0.62,
      score: 0.55 + i * 0.01,
      at: new Date().toISOString(),
    }));
    events.emit(
      'memory.injector_run' as never,
      {
        sessionId: 's',
        activated: [{ id: 'mem-1', kind: 'fact' }],
        injected: [],
        rejectedDetail: rejected,
        rejectedDetailTotal: 20,
      } as never,
    );
    const recorded = await journal.readAll();
    off();
    const attrs = recorded[0]!.attributes as Record<string, unknown>;
    const rejectedDetail = attrs['rejectedDetail'] as {
      items: Array<Record<string, unknown>>;
      total: number;
      truncated: true;
    };
    expect(rejectedDetail.items).toHaveLength(5);
    expect(rejectedDetail.total).toBe(20);
    expect(rejectedDetail.truncated).toBe(true);
    // FIFO eviction — oldest entries first; the first 5 (indices 0–4) survive.
    expect((rejectedDetail.items[0] as Record<string, unknown>).id).toBe('mem-rej-0');
    expect((rejectedDetail.items[4] as Record<string, unknown>).id).toBe('mem-rej-4');
  });

  it('dedupes memory.injector_run entries that are both activated and injected', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-memdedup-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    const context = createChronicleContext({ installationId: 'i', machineId: 'm' }, 'trace');
    ...
    expect(injected).toEqual([
      { id: 'mem-1', ref: 'activated' },
      { id: 'mem-2', kind: 'fact', score: 0.5 },
    ]);
  ...
  it('persists file.event mutations with task/board scope and skips reads', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-filev-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    const context = createChronicleContext(
    ...
    events.emit(
      'kanban.task_moved' as never,
      { sessionId: 's', taskId: 'task-9', boardId: 'board-3', from: 'doing', to: 'done' } as never,
    );
    const recorded = await journal.readAll();
    off();
  ...
  it('extracts provider/model into runtime from any domain event payload', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'chronicle-domain-runtime-'));
    dirs.push(dir);
    const journal = new ChronicleJournal({ filePath: path.join(dir, 'events.jsonl') });
    const events = new EventBus();
    const context = createChronicleContext(
    ...
    expect((recorded[0]!.attributes as Record<string, unknown>).model).toBe(
