...
describe('/sync verbs', () => {
  ...
  it('enable: usage, invalid repo, and success (encrypts + writes + updates)', async () => {
    ...
    expect((await runCmd(cmd, 'enable me/data ghp_x bogus', ctx)).message).toContain('Unknown categories');
  ...
  it('push: not-enabled, no-token, success, and failure', async () => {
    expect((await runCmd(build(() => ({ sync: { enabled: false } })).cmd, 'push', ctx)).message).toContain('not enabled');
    expect((await runCmd(build(() => ({ sync: { enabled: true } })).cmd, 'push', ctx)).message).toContain('No GitHub token');
    ...
    expect((await runCmd(build(() => ({ sync: { enabled: true, githubToken: 't' } })).cmd, 'push', ctx)).message).toContain('Push failed: network down');
  });

  it('pull: not-enabled, no-token, success, and failure', async () => {
    expect((await runCmd(build(() => ({ sync: { enabled: false } })).cmd, 'pull', ctx)).message).toContain('not enabled');
    expect((await runCmd(build(() => ({ sync: { enabled: true } })).cmd, 'pull', ctx)).message).toContain('No GitHub token');

    const ok = build(() => ({ sync: { enabled: true, githubToken: 't', repo: 'r', categories: [] } }));
    expect((await runCmd(ok.cmd, 'pull', ctx)).message).toBe('pulled ok');

    cloudInstance.pull.mockRejectedValueOnce(new Error('boom'));
    expect((await runCmd(build(() => ({ sync: { enabled: true, githubToken: 't' } })).cmd, 'pull', ctx)).message).toContain('Pull failed: boom');
  });

  it('categories: gated, list, add (unknown/dup/success), remove (missing/success), usage', async () => {
    expect((await runCmd(build(() => ({ sync: { enabled: false } })).cmd, 'categories', ctx)).message).toContain('not enabled');

    const enabled = (cats: string[]) => build(() => ({ sync: { enabled: true, categories: cats } }));

    expect((await runCmd(enabled(['memory']).cmd, 'categories', ctx)).message).toContain('Synced categories: memory');
    expect((await runCmd(enabled(['memory']).cmd, 'categories list', ctx)).message).toContain('Available:');

    expect((await runCmd(enabled(['memory']).cmd, 'categories add bogus', ctx)).message).toContain('Unknown');
    expect((await runCmd(enabled(['memory']).cmd, 'categories add memory', ctx)).message).toContain('already synced');
    const added = enabled(['memory']);
    expect((await runCmd(added.cmd, 'categories add prompts', ctx)).message).toContain('Added "prompts"');
    ...
    expect((await runCmd(enabled(['memory']).cmd, 'categories remove prompts', ctx)).message).toContain('not in sync');
    const removed = enabled(['memory', 'prompts']);
    expect((await runCmd(removed.cmd, 'categories remove prompts', ctx)).message).toContain('Removed "prompts"');

    expect((await runCmd(enabled(['memory']).cmd, 'categories frobnicate', ctx)).message).toContain('Usage');
  });
