...
describe('brain-runtime apply() validation', () => {
  ...
  it('rejects invalid maxAutoRisk', () => {
    const r = rt();
    expect(() => r.apply({ maxAutoRisk: 'extreme' as never }, { persist: false })).toThrow(
      'Invalid maxAutoRisk',
    );
  });
  ...
  it('rejects invalid strategy', () => {
    const r = rt();
    expect(() => r.apply({ strategy: 'random' as never }, { persist: false })).toThrow(
      'Invalid strategy',
    );
  });
  ...
  it('rejects non-positive decisionTimeoutMs', () => {
    const r = rt();
    expect(() => r.apply({ decisionTimeoutMs: 0 }, { persist: false })).toThrow(
      'Invalid decisionTimeoutMs',
    );
  });
  ...
  it('rejects non-array rules', () => {
    const r = rt();
    expect(() => r.apply({ rules: 'not-array' as never }, { persist: false })).toThrow(
      'Invalid rules',
    );
  });
  ...
  it('rejects invalid council minRisk', () => {
    const r = rt();
    expect(() => r.apply({ council: { minRisk: 'low' as never } }, { persist: false })).toThrow(
      'Invalid council minRisk',
    );
  });

  it('rejects invalid council quorum', () => {
    const r = rt();
    expect(() => r.apply({ council: { quorum: 1.5 } }, { persist: false })).toThrow(
      'Invalid council quorum',
    );
    expect(() => r.apply({ council: { quorum: 0 } }, { persist: false })).toThrow(
      'Invalid council quorum',
    );
  });
  ...
  it('rejects invalid council approval', () => {
    const r = rt();
    expect(() => r.apply({ council: { approval: -0.1 } }, { persist: false })).toThrow(
      'Invalid council approval',
    );
  });

  it('rejects invalid council integer fields', () => {
    const r = rt();
    expect(() => r.apply({ council: { perCallTimeoutMs: -1 } }, { persist: false })).toThrow(
      'Invalid council',
    );
    expect(() => r.apply({ council: { maxConcurrency: 0 } }, { persist: false })).toThrow(
      'Invalid council',
    );
    expect(() => r.apply({ council: { judgeMaxTokens: 1.5 } }, { persist: false })).toThrow(
      'Invalid council',
    );
    expect(() => r.apply({ council: { voterMaxTokens: 0 } }, { persist: false })).toThrow(
      'Invalid council',
    );
  });
  ...
  it('rejects non-array council seats', () => {
    const r = rt();
    expect(() => r.apply({ council: { seats: 'not-array' as never } }, { persist: false })).toThrow(
      'Invalid council.seats',
    );
  });

  it('rejects council seats without persona', () => {
    const r = rt();
    expect(() => r.apply({ council: { seats: [{ persona: '' }] } }, { persist: false })).toThrow(
      'Invalid council.seats',
    );
  });
  ...
  it('rejects invalid terminalPolicy', () => {
    const r = rt();
    expect(() => r.apply({ terminalPolicy: 'yolo' as never }, { persist: false })).toThrow(
      'Invalid terminalPolicy',
    );
  });
  ...
  it('rejects non-positive decisionLogMaxEntries', () => {
    const r = rt();
    expect(() => r.apply({ decisionLogMaxEntries: 0 }, { persist: false })).toThrow(
      'Invalid decisionLogMaxEntries',
    );
    expect(() => r.apply({ decisionLogMaxEntries: 1.5 }, { persist: false })).toThrow(
      'Invalid decisionLogMaxEntries',
    );
  });
  ...
  it('rejects non-boolean cache.enabled', () => {
    const r = rt();
    expect(() => r.apply({ cache: { enabled: 'yes' as never } }, { persist: false })).toThrow(
      'Invalid cache.enabled',
    );
  });

  it('rejects invalid cache.ttlMs', () => {
    const r = rt();
    expect(() => r.apply({ cache: { ttlMs: 0 } }, { persist: false })).toThrow(
      'Invalid cache.ttlMs',
    );
  });
  ...
  it('rejects invalid cache.maxEntries', () => {
    const r = rt();
    expect(() => r.apply({ cache: { maxEntries: -1 } }, { persist: false })).toThrow(
      'Invalid cache.maxEntries',
    );
  });
  ...
  it('rejects invalid llm.maxTokens', () => {
    const r = rt();
    expect(() => r.apply({ llm: { maxTokens: 0 } }, { persist: false })).toThrow(
      'Invalid llm.maxTokens',
    );
  });

  it('rejects non-boolean llm.rejectUncertain', () => {
    const r = rt();
    expect(() => r.apply({ llm: { rejectUncertain: 'yes' as never } }, { persist: false })).toThrow(
      'Invalid llm.rejectUncertain',
    );
  });
  ...
  it('rejects invalid llm.minConfidence', () => {
    const r = rt();
    expect(() => r.apply({ llm: { minConfidence: 1.5 } }, { persist: false })).toThrow(
      'Invalid llm.minConfidence',
    );
    expect(() => r.apply({ llm: { minConfidence: -0.1 } }, { persist: false })).toThrow(
      'Invalid llm.minConfidence',
    );
  });
  ...
  it('rejects non-boolean trace.enabled', () => {
    const r = rt();
    expect(() => r.apply({ trace: { enabled: 'yes' as never } }, { persist: false })).toThrow(
      'Invalid trace.enabled',
    );
  });

  it('rejects invalid trace.content', () => {
    const r = rt();
    expect(() => r.apply({ trace: { content: 'verbose' as never } }, { persist: false })).toThrow(
      'Invalid trace.content',
    );
  });

  it('rejects invalid trace.maxOpenRecords', () => {
    const r = rt();
    expect(() => r.apply({ trace: { maxOpenRecords: 0 } }, { persist: false })).toThrow(
      'Invalid trace.maxOpenRecords',
    );
  });
  ...
  it('rejects invalid ledger.autoDenyAfterFailures', () => {
    const r = rt();
    expect(() => r.apply({ ledger: { autoDenyAfterFailures: -1 } }, { persist: false })).toThrow(
      'Invalid autoDenyAfterFailures',
    );
  });
  ...
  it('rejects invalid model refs', () => {
    const r = rt();
    expect(() => r.apply({ models: [{ model: '' }] }, { persist: false })).toThrow(
      'Invalid model ref',
    );
  });
  ...
  it('accepts monitor patch', () => {
    const r = rt();
    expect(() => r.apply({ monitor: { stallMs: 30000 } }, { persist: false })).not.toThrow();
  });
});
