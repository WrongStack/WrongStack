...
function fakeProvider(id: string, opts?: { failCount?: number; model?: string; maxContext?: number }): Provider {
  let calls = 0;
  return {
    id,
    capabilities: {
...
function fakeProviderThatThrows(id: string, kind: ProviderErrorKind = 'overloaded'): Provider {
  return {
    id,
    capabilities: { maxContext: 128_000, tools: true, parallelTools: false, vision: false, streaming: false, promptCache: false, systemPrompt: true, jsonMode: false, reasoning: false, cacheControl: 'none' },
...
function makeOneShotOpts(config: Config, buildProvider: OneShotOrchestratorOptions['buildProvider']) {
  return {
    buildProvider,
    getConfig: () => config,
...
describe('OneShotOrchestrator context-window pre-filter', () => {
  it('skips a fallback provider whose maxContext is smaller than the estimated request tokens', async () => {
    ...
    const smallProvider = fakeProvider('small-provider', { model: 'small-model', maxContext: 4_000 });
    const largeProvider = fakeProvider('large-provider', { model: 'large-model', maxContext: 200_000 });
  ...
  it('does NOT skip fallback entries when the context window is large enough', async () => {
    const primary = fakeProviderThatThrows('primary', 'overloaded');
    const fallback = fakeProvider('fallback-provider', { model: 'fallback-model', maxContext: 128_000 });
  ...
  it('skips all fallback entries when none have sufficient context window', async () => {
    ...
    const smallProvider = fakeProvider('small-provider', { model: 'small-model', maxContext: 1_000 });
  ...
  it('does NOT skip when maxContext is unknown (0 or undefined)', async () => {
    ...
    const unknownProvider: Provider = {
      id: 'unknown-provider',
      capabilities: { maxContext: 0, tools: true, parallelTools: false, vision: false, streaming: false, promptCache: false, systemPrompt: true, jsonMode: false, reasoning: false, cacheControl: 'none' },
