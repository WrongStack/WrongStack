...
describe('favorite_manage — remove', () => {
  ...
  it('rejects index 0', async () => {
    const tool = createFavoriteManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove', index: 0 }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
    expect(result.message).toContain('out of range');
  });
  ...
  it('rejects removing a model not in favorites', async () => {
    const tool = createFavoriteManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove', model: 'openai/nonexistent' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
...
describe('favorite_manage — unknown action', () => {
  it('returns an error for unrecognized actions', async () => {
    const tool = createFavoriteManageTool(makeOpts());
    const result = await tool.execute({ action: 'destroy' as never }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
