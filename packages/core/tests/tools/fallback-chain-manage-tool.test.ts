...
describe('fallback_chain_manage — list', () => {
  it('lists the chain with positions', async () => {
    ...
    const result = await tool.execute({ action: 'list' }, {} as never, { signal: new AbortController().signal });
...
describe('fallback_chain_manage — add', () => {
  it('appends a favorite model to the chain', async () => {
    ...
    const result = await tool.execute({ action: 'add', model: 'openai/gpt-4o' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
  ...
  it('rejects adding without a model', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'add' }, {} as never, { signal: new AbortController().signal });
  ...
  it('rejects adding a non-favorite model', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'add', model: 'anthropic/claude-3' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
  ...
  it('rejects adding a duplicate', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'add', model: 'test-provider/test-model' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
...
describe('fallback_chain_manage — insert', () => {
  it('inserts at a specific position', async () => {
    const opts = makeOpts();
    const tool = createFallbackChainManageTool(opts);
    const result = await tool.execute({ action: 'insert', model: 'openai/gpt-4o', index: 1 }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
  ...
  it('appends when index is omitted', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'insert', model: 'openai/gpt-4o' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
  ...
  it('clamps index to valid range', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'insert', model: 'openai/gpt-4o', index: 999 }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
    // Clamped to end
    expect(result.chain?.[result.chain!.length - 1]).toBe('openai/gpt-4o');
  });
  ...
  it('rejects insert without model', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'insert' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
  });

  it('rejects inserting a non-favorite', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'insert', model: 'anthropic/claude-3' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
    expect(result.message).toContain('not in your favorites');
  });
  ...
  it('rejects inserting a duplicate', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'insert', model: 'test-provider/test-model' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
    expect(result.message).toContain('already in the chain');
  });
});
...
describe('fallback_chain_manage — remove', () => {
  it('removes by 1-based index', async () => {
    const opts = makeOpts();
    const tool = createFallbackChainManageTool(opts);
    const result = await tool.execute({ action: 'remove', index: 1 }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
  ...
  it('rejects out-of-range index', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove', index: 99 }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
  ...
  it('removes by model ref', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove', model: 'test-provider/other-model' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
  ...
  it('rejects removing a model not in chain', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove', model: 'openai/gpt-4o' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
  ...
  it('rejects remove without model or index', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'remove' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
...
describe('fallback_chain_manage — clear', () => {
  it('clears a non-empty chain', async () => {
    const opts = makeOpts();
    const tool = createFallbackChainManageTool(opts);
    const result = await tool.execute({ action: 'clear' }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('ok');
...
describe('fallback_chain_manage — unknown action', () => {
  it('returns an error', async () => {
    const tool = createFallbackChainManageTool(makeOpts());
    const result = await tool.execute({ action: 'nuke' as never }, {} as never, { signal: new AbortController().signal });
    expect(result.status).toBe('error');
