/**
 * Teardown helper for tests that touch a project mailbox.
 *
 ...
 *
 * Call {@link disposeProjectMailbox} in `afterEach`, before removing the temp
 * directory. It closes the local wrapper (if any), asks the owner to exit, and
 * waits for the socket to go away.
 */
...
export async function disposeProjectMailboxesUnder(globalRoot: string): Promise<void> {
  let entries: string[];
  try {
    entries = (await fs.readdir(path.join(globalRoot, 'projects'), { withFileTypes: true }))
      .filter((entry) => entry.isDirectory())
      .map((entry) => path.join(globalRoot, 'projects', entry.name));
  } catch {
    return; // No project state was ever written under this root.
  }
  await Promise.all(entries.map((dir) => disposeProjectMailbox(dir)));
}
