...
/**
 * Dedicated vitest config for `tests/status-bar-overflow.test.ts`.
 *
 * The orange-styling pin in that file asserts the raw `\x1b[38;2;253;159;2m`
 ...
 * downsamples `#FD9F02` to a basic ANSI code rather than the expected truecolor.
 *
 * `FORCE_COLOR=3` + `COLORTERM=truecolor` force chalk to its truecolor level
 * (24-bit) so `chalk.hex('#FD9F02')` emits exactly the escape the assertion
 * pins. These are set at worker start so chalk's auto-detection picks them up
 * at initialization.
 *
