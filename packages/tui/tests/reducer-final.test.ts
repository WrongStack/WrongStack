...
describe('reducer — final correct branches', () => {
  ...
  it('toggleSddBoardMonitor flips when sddBoard is set', () => {
    const s = initial({ sddBoard: { snapshot: { columns: [], tasks: [], edges: [], phases: [], decisions: [] }, monitorOpen: false } } as never);
  ...
  it('modelPickerSearch filters when step=model', () => {
    const s = initial({ modelPicker: { open: true, step: 'model' as const, providerOptions: [], modelOptions: ['claude-3', 'gpt-4'], filteredOptions: ['claude-3', 'gpt-4'], selected: 0, searchQuery: '' } } as never);
  ...
  it('authPromptStart/Change/End cycle uses input field', () => {
    let s = initial({ authPanel: { open: true, view: 'flow' as const, providers: [], presets: [], catalog: [], selected: 0, filter: '', busy: true, hint: '', log: [], flowTitle: 'Test', flowDone: false } } as never);
  ...
  it('resumePickerBusy/Error work when open', () => {
    let s = reducer(initial(), { type: 'resumePickerOpen', sessions: [{ id: 's1', startedAt: '2026-01-01T00:00:00Z', title: 'Test', tokenTotal: 0, toolCallCount: 0, iterationCount: 0, toolErrorCount: 0, outcome: 'completed', isCurrent: false }] });
