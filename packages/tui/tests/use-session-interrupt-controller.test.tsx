...
interface MockRefs {
  ...
  tokenPreviewsRef: { current: { clear: ReturnType<typeof vi.fn> } };
  clearPendingConfirms: ReturnType<typeof vi.fn>;
}
...
function buildRefs(status: State['status'] = 'idle'): MockRefs {
  ...
  return {
    ...
    tokenPreviewsRef: { current: { clear: vi.fn() } },
...
describe('useSessionInterruptController', () => {
  ...
  describe('resetSession', () => {
    ...
    it('dispatches streamReset, toolStreamClear, and queueClear', () => {
      ...
      const types = refs.dispatch.mock.calls.map((c: [Action]) => c[0].type);
