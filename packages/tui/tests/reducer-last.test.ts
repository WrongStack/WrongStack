...
describe('reducer — last branches', () => {
  ...
  it('collabSubagentSpawned bootstraps collabSession', () => {
    const r = reducer(initial(), { type: 'collabSubagentSpawned', role: 'bug-hunter', goal: 'find bugs' } as never);
  ...
  it('collabSubagentSpawned is no-op when collabSession exists', () => {
    const s = reducer(initial(), { type: 'collabSubagentSpawned', role: 'hunter', goal: 'hunt' } as never);
  ...
  it('collabBugFound bootstraps with description', () => {
    const r = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'null pointer', severity: 'high' } as never);
    expect(r.collabSession).not.toBeNull();
    expect(r.collabSession?.bugCount).toBe(1);
  });
  it('collabBugFound appends to existing', () => {
    const s = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'bug1', severity: 'high' } as never);
    const r = reducer(s, { type: 'collabBugFound', sessionId: 's1', description: 'bug2', severity: 'medium' } as never);
    expect(r.collabSession?.bugCount).toBe(2);
  });
  it('collabSessionDone sets verdict', () => {
    const s = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'bug', severity: 'low' } as never);
    const r = reducer(s, { type: 'collabSessionDone', sessionId: 's1', verdict: 'approve', score: 9 } as never);
    expect(r.collabSession?.overallVerdict).toBe('approve');
  });
  it('collabSessionDone no-op when no session', () => {
    const r = reducer(initial(), { type: 'collabSessionDone', sessionId: 's1', verdict: 'approve', score: 9 } as never);
