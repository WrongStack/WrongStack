...
export const ESC_CLOSE_PANELS: readonly EscCloseEntry[] = [
  ...
  { name: 'agentsMonitor', isOpen: (s) => s.agentsMonitorOpen, close: { type: 'toggleAgentsMonitor' } },
  { name: 'fleetMonitor', isOpen: (s) => s.monitorOpen, close: { type: 'toggleMonitor' } },
  // Non-modal overlays (input stays live behind these).
  { name: 'todosMonitor', isOpen: (s) => s.todosMonitorOpen, close: { type: 'toggleTodosMonitor' } },
  ...
  { name: 'coordinator', isOpen: (s) => s.coordinator.monitorOpen, close: { type: 'toggleCoordinatorMonitor' } },
  { name: 'sessionsPanel', isOpen: (s) => s.sessionsPanelOpen, close: { type: 'toggleSessionsPanel' } },
  // Modal pickers (also handled by tryPickerKey — safety net).
  { name: 'settingsPicker', isOpen: (s) => s.settingsPicker.open, close: { type: 'settingsClose' } },
  { name: 'projectPicker', isOpen: (s) => s.projectPicker.open, close: { type: 'projectPickerClose' } },
 ...
 * F-key toggles, busy-interrupt) has consumed the key.
 */
