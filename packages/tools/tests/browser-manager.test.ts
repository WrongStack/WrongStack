...
describe.skipIf(!playwrightAvailable)(
  'BrowserSessionManager Playwright fixture integration',
  () => {
    ...
    it('isolates sessions by agent owner', async () => {
      ...
      try {
        ...
        await expect(manager.snapshot(opened.id, 'agent-b', signal)).rejects.toThrow(
          /another agent/,
        );
