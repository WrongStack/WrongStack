...
describe('every canonical preset is selectable', () => {
  ...
  it('exposes 35 presets — the count is pinned so a silent drop is visible', () => {
    expect(THEME_PRESET_IDS).toHaveLength(35);
    expect(new Set(THEME_PRESET_IDS).size).toBe(35);
  });
