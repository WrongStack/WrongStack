...
const CORPUS: Case[] = [
  ...
  },
  {
    ...
    board: board([task('epoch-due', { dueDate: 1_784_000_000_000 } as Partial<KanbanTask>)]),
    requireDueDate: true,
  },
  {
    label: 'due date stored as a Date',
    board: board([task('date-due', { dueDate: new Date(NOW) } as Partial<KanbanTask>)]),
    requireDueDate: true,
  },
  {
  ...
  },
  {
    ...
    board: board([task('string-criteria', { successCriteria: ['Ships'] } as Partial<KanbanTask>)]),
  },
  {
    label: 'string success criteria on a managed board',
    board: managed([
      task('managed-string-criteria', { successCriteria: ['Ships'] } as Partial<KanbanTask>),
    ]),
  },
  {
