...
vi.mock('../src/subcommands/handlers/modeldiag-profiles.js', () => ({
  ...
  EVAL_TASKS: { coding: { label: 'Code', prompt: 'Write code.' }, planning: { label: 'Plan', prompt: 'Plan.' } },
}));

vi.mock('@wrongstack/providers', () => ({
  makeProviderFromConfig: vi.fn(() => ({ id: 'openai', capabilities: { maxContext: 128000 }, complete: vi.fn() })),
  setOAuthTokenPersister: vi.fn(),
}));

vi.mock('../src/boot/auto-discover-providers.js', () => ({ discoverAndMergeProviders: vi.fn(async () => undefined) }));
vi.mock('../src/provider-config-utils.js', () => ({
  mutateConfigProviders: vi.fn(async () => undefined),
  normalizeKeys: vi.fn(() => []),
  writeKeysBack: vi.fn(),
}));
vi.mock('../src/profile-config-path.js', () => ({ activeProfileConfigPath: vi.fn(() => '/tmp/cfg.json') }));
vi.mock('../src/subcommands/handlers/model-smoke-test.js', () => ({
  buildModelSmokeTargets: vi.fn(async () => []),
  parseModelSmokeOptions: vi.fn(() => ({ planOnly: false, allModels: false, timeoutMs: 45000, maxTokens: 32, json: false, providerIds: undefined, modelIds: undefined })),
...
function makeDeps() {
  ...
  return {
    lines,
    deps: {
      renderer: { write: vi.fn((s: string) => { lines.push(s); }) },
      config: {
        provider: 'openai',
        model: 'gpt-4o',
...
describe('modeldiag handler', () => {
  ...
  it('test --plan shows smoke test plan', async () => {
    const { deps, lines } = makeDeps();
    const { buildModelSmokeTargets } = await import('../src/subcommands/handlers/model-smoke-test.js');
