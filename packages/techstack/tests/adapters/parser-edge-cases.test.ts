...
describe('adapter parser edge cases', () => {
  it('parses pnpm importer comments, commit resolutions, links, and CRLF', async () => {
    const deps = await inventory(npmAdapter, 'npm', {
  ...
  it('parses Python multiline arrays, inline tables, and quoted keys', async () => {
    const deps = await inventory(pythonAdapter, 'python', {
      'pyproject.toml': [
        '[project]',
        'dependencies = [',
        '  "requests>=2.31", # HTTP',
    ...
    expect(deps.map((dep) => dep.name)).toEqual(expect.arrayContaining([
      'requests', 'flask', 'pytest', 'typing-extensions',
    ]));
  });

  it('parses Cargo git sources and duplicate package names deterministically', async () => {
    const deps = await inventory(rustAdapter, 'rust', {
      'Cargo.toml': '[dependencies]\nserde = "1"\nfoo = { git = "https://example.test/foo", tag = "v1" }\n',
      'Cargo.lock': [
        '[[package]]', 'name = "serde"', 'version = "1.0.200"',
        'source = "registry+https://github.com/rust-lang/crates.io-index"',
        '[[package]]', 'name = "foo"', 'version = "1.2.0"',
  ...
  it('handles go.mod replace, retract, exclude, pseudo-versions, and CRLF', async () => {
    ...
    const deps = await inventory(goAdapter, 'go', {
      'go.mod': [
        'module example.test/app', 'require (', `  example.test/pseudo ${pseudo}`,
        '  example.test/direct v1.2.0', ')',
        'replace example.test/direct => ../direct',
        'exclude example.test/direct v1.1.0', 'retract [v1.0.0, v1.0.1]',
      ].join('\r\n'),
      'go.sum': `example.test/pseudo ${pseudo} h1:abc\r\nexample.test/direct v1.2.0 h1:def\r\n`,
    }, ['go.mod'], ['go.sum']);

    expect(deps.find((dep) => dep.name === 'example.test/pseudo')?.locked)
      .toBe('0.0.0-20240701012345-abcdefabcdef');
  ...
  it('classifies Flutter SDK, dependency overrides, and git refs', async () => {
    const deps = await inventory(dartAdapter, 'dart', {
      'pubspec.yaml': [
        'dependencies:', '  flutter:', '    sdk: flutter',
        '  http: ^1.2.0', '  git_pkg:', '    git:',
        '      url: https://example.test/pkg.git', '      ref: stable',
        'dependency_overrides:', '  http: ^1.3.0',
      ].join('\n'),
      'pubspec.lock': [
        'packages:', '  http:', '    dependency: direct main', '    version: "1.3.0"',
        '  git_pkg:', '    dependency: direct main', '    version: "2.0.0"',
      ].join('\n'),
  ...
  it('parses csproj attributes in any order and nested Version elements', async () => {
    const deps = await inventory(dotNetAdapter, 'dotnet', {
      'app.csproj': [
        '<Project><ItemGroup>',
  ...
  it('resolves Maven properties, parent properties, and dependencyManagement', async () => {
    const deps = await inventory(mavenAdapter, 'maven', {
      'pom.xml': [
    ...
    expect(deps.find((dep) => dep.name === 'org.junit.jupiter:junit-jupiter')?.requested).toBe('5.10.2');
  ...
  it('reads Ruby GEM, GIT, and PATH lock sections', async () => {
    const deps = await inventory(rubyAdapter, 'ruby', {
      Gemfile: "gem 'rack'\ngem 'gitgem', git: 'https://example.test/gitgem'\ngem 'localgem', path: '../local'\n",
      'Gemfile.lock': [
        'GIT', '  remote: https://example.test/gitgem', '  specs:', '    gitgem (1.2.0)',
        'PATH', '  remote: ../local', '  specs:', '    localgem (0.3.0)',
        'GEM', '  remote: https://rubygems.org/', '  specs:', '    rack (3.1.0)',
      ].join('\n'),
  ...
  it('ignores Composer abandoned and notification metadata while parsing packages', async () => {
    const deps = await inventory(phpAdapter, 'php', {
      'composer.json': JSON.stringify({ require: { 'vendor/pkg': '^1.0' } }),
      'composer.lock': JSON.stringify({
        packages: [{ name: 'vendor/pkg', version: '1.2.3', abandoned: true, 'notification-url': 'https://example.test' }],
      }),
  ...
  it('classifies Elixir git tuples without treating tags as registry constraints', async () => {
    const deps = await inventory(elixirAdapter, 'elixir', {
      'mix.exs': 'defp deps do\n[{:plug, "~> 1.16"}, {:git_dep, git: "https://example.test/git_dep", tag: "1.0.0"}]\nend\n',
      'mix.lock': '%{"plug" => {:hex, :plug, "1.16.1"}, "git_dep" => {:git, "https://example.test/git_dep", "abc", [tag: "1.0.0"]}}',
    }, ['mix.exs'], ['mix.lock']);
