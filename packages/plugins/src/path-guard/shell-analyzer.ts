...
export function firstUnquotedShellSeparator(raw: string, start: number, end: number): number | undefined {
  let quote: "'" | '"' | null = null;
...
export function isQuoteBoundary(command: string, index: number, activeQuote: "'" | '"' | null): boolean {
  const char = command[index];
