  ...
  for (const anchor of memory.anchors) {
    ...
    if (isPrefix) {
      ...
      if (!best || decayed > best.strength) {
        best = { strength: decayed, reason: `anchor:ancestor:${anchorNorm}+${distance}` };
      }
    }
  }
  return best;
...
export function applyCooldown(
  ...
  return memories.filter((memory) => {
    ...
    if (repeatCooldownMs <= 0 || !Number.isFinite(repeatCooldownMs)) return false;
    return now - last >= repeatCooldownMs;
  });
}

export function cooldownKey(memoryId: string, sessionId?: string | undefined): string {
...
export function containsMemoryText(visibleContext: string, memoryText: string): boolean {
