...
const { HQ_BROWSER_PEER_RESUME_CLIENT_ID, HqWsClient, getHqClient, closeHqClient } = await import(
  '../src/lib/hq-ws-client.js'
);
...
describe('HqWsClient', () => {
  ...
  afterEach(() => {
    ...
    // leak via `getHqClient` and trip the conflict-detection warning.
    closeHqClient();
  ...
  it('ignores delayed close events from sockets replaced by reconnect', () => {
    client = new HqWsClient({ url: 'ws://test/ws', heartbeatIntervalMs: 1, heartbeatTimeoutMs: 1 });
    client.connect();
  ...
  it('sends no client.resume frame on open when there is no stored cursor', () => {
    client = new HqWsClient({ url: 'ws://test/ws', resumeCursor: () => ({}) });
    client.connect();
    currentWs!._open();
  ...
  it('uses the latest resume cursor value on reconnect', () => {
    let seq = 1;
    client = new HqWsClient({ url: 'ws://test/ws', resumeCursor: () => ({ 'client-a': seq }) });
    client.connect();
    currentWs!._open();
  ...
  it('still starts heartbeat when sending client.resume fails on open', () => {
    client = new HqWsClient({ url: 'ws://test/ws', resumeCursor: () => ({ 'client-a': 1 }) });
    client.connect();
  ...
  it('getHqClient returns the same instance on repeat calls', () => {
    const a = getHqClient();
    const b = getHqClient();
    expect(a).toBe(b);
    a.close();
  ...
  it('closeHqClient tears down the cached singleton so the next call builds a fresh one', () => {
    const a = getHqClient();
    closeHqClient();
  ...
  it('closeHqClient is idempotent', () => {
    const a = getHqClient();
    closeHqClient();
    expect(() => closeHqClient()).not.toThrow();
    expect(() => closeHqClient()).not.toThrow();
    const b = getHqClient();
    expect(b).not.toBe(a);
    b.close();
  ...
  it('accepts a Map-backed resume cursor (cross-realm safe)', () => {
    ...
    const localClient = new HqWsClient({
      url: 'ws://test-map/ws',
