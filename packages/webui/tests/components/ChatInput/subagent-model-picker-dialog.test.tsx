...
vi.mock('@/hooks/useWebSocket', () => ({
  useWebSocket: () => ({ listSavedProviders, listProviderModels }),
}));
vi.mock('@/stores', () => ({
  useConfigStore: (selector: (s: unknown) => unknown) => selector({ wsUrl: 'ws://test' }),
}));
vi.mock('@/stores/local-prefs', () => ({
  useLocalPrefs: (selector: (s: unknown) => unknown) =>
    selector({ favoriteModels: ['openai/gpt-5'], disabledModels: [] }),
}));
vi.mock('@/lib/ws-client', () => ({
  getWSClient: () => ({
    on: (event: string, callback: (message: { payload: unknown }) => void) => {
...
function openDialog(currentLane: Record<string, string> = {}) {
  ...
  act(() => {
    listeners['providers.saved']?.({
      payload: { providers: [{ id: 'openai' }, { id: 'anthropic' }] },
    });
    listeners['provider.models']?.({
      payload: {
        provider: 'openai',
        models: [
          { id: 'gpt-5', name: 'GPT-5' },
    ...
    listeners['provider.models']?.({
      payload: { provider: 'anthropic', models: [{ id: 'claude', name: 'Claude' }] },
    });
  });
}
...
describe('SubagentModelPickerDialog', () => {
  ...
  it('toggles favorites-only filtering', () => {
    openDialog();
    fireEvent.click(screen.getByRole('button', { name: /favorites/i }));
    expect(screen.getByText('GPT-5')).toBeTruthy();
    expect(screen.queryByText('Claude')).toBeNull();
  });
  it('filters candidates by provider', () => {
    openDialog();
  ...
  it('supports keyboard navigation and Enter selection', async () => {
    ...
    act(() => {
      listeners['providers.saved']?.({ payload: { providers: [{ id: 'openai' }] } });
      listeners['provider.models']?.({
        payload: { provider: 'openai', models: [{ id: 'gpt-5', name: 'GPT-5' }] },
      });
    });
    ...
    expect(onPick).toHaveBeenCalledWith({ provider: 'openai', model: 'gpt-5' });
  });
  it('shows a warning for legacy tier pins', () => {
    openDialog({ tier: 'budget' });
    expect(screen.getByText(/tier:budget/)).toBeTruthy();
    expect(screen.getByText(/replaces this pin/)).toBeTruthy();
  });
});
