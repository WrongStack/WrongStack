...
describe('SubagentModelPickerDialog', () => {
  ...
  it('supports keyboard navigation and Enter selection', async () => { const onPick = vi.fn(); render(<SubagentModelPickerDialog laneIndex={0} currentLane={{}} open onOpenChange={vi.fn()} onPick={onPick} />); await waitFor(() => expect(listSavedProviders).toHaveBeenCalled()); act(() => { listeners['providers.saved']?.({ payload: { providers: [{ id: 'openai' }] } }); listeners['provider.models']?.({ payload: { provider: 'openai', models: [{ id: 'gpt-5', name: 'GPT-5' }] } }); }); await waitFor(() => expect(screen.getByText('GPT-5')).toBeTruthy()); const input = screen.getByRole('textbox'); fireEvent.keyDown(input, { key: 'ArrowDown' }); fireEvent.keyDown(input, { key: 'Enter' }); expect(onPick).toHaveBeenCalledWith({ provider: 'openai', model: 'gpt-5' }); });
