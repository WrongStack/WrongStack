...
export const pluginDetailsPart1: Record<string, PluginDetail> = {
  ...
  'wstack-sync': {
    version: '1.0.0',
    longDescription:
      'Built-in cloud sync plugin. Registers the /sync slash command (status, enable, disable, push, pull, categories) that synchronizes prompts, skills, settings, memory, and history with a GitHub-backed store. The plugin stays passive until sync is explicitly enabled and configured.',
    tools: [],
    configOptions: [],
    hooks: [],
    apiVersion: '^0.1',
  ...
  'wstack-chimera': {
    ...
    configOptions: [
      ...
      },
      {
        ...
        defaultValue: 'off',
        description:
          'How to handle review findings: off = notify leader (wait for user command), ask = prompt user for permission first, auto = fix findings immediately with a dedicated subagent.',
      },
      {
        name: 'cascadeOn',
  ...
  'auto-i18n-extractor': {
    ...
    configOptions: [
      ...
      },
      {
        ...
        type: 'string[]',
        defaultValue:
          '["className","class","testId","data-testid","aria-label","aria-labelledby","key","id","name","htmlFor"]',
  ...
  'accessibility-auditor': {
    version: '0.1.0',
    longDescription:
      'Audits UI files for common accessibility issues using fast regex heuristics. The a11y_audit tool scans a file or directory on demand, and a PostToolUse hook on write/edit summarizes any new findings for the file just changed. Severity "warn" injects context; "block" refuses the edit.',
    tools: [
      {
        name: 'a11y_audit',
        category: 'Diagnostics',
