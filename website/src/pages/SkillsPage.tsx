  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10 lg:py-36">
        <SectionIntro index="01" eyebrow="How they work" title="Trigger words activate skills automatically." description="Every skill declares a set of trigger words. When your message contains one, the skill's full instruction body is injected into the agent's context — no command needed." />
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-line bg-line lg:grid-cols-3">
          {[
            ...
            { step: '03', title: 'Agent applies it', body: 'The agent follows the skill\'s detailed instructions alongside its baseline rules. The skill deactivates after the turn unless triggered again.' },
          ].map(({ step, title, body }) => (<article key={step} className="bg-card p-7"><span className="font-mono text-xs font-black text-brand-2">{step}</span><h2 className="mt-8 text-xl font-black text-fg">{title}</h2><p className="mt-3 text-sm leading-7 text-muted">{body}</p></article>))}
        </div>
      </section>

      <section className="border-y border-line bg-surface">
        <div className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10">
          <SectionIntro index="02" eyebrow="Lifecycle" title="Seven commands for the full skill lifecycle." />
      ...
      <section className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10 lg:py-36">
        <SectionIntro index="03" eyebrow="Sources" title="Three layers of skill resolution." description="WrongStack discovers skills from multiple locations. Layering means you can override a bundled skill with your own version without touching shipped files." />
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {[
            { icon: PackageOpen, title: 'Bundled', path: 'Shipped with @wrongstack/core', body: '50+ skills ship with WrongStack: security scanners, testing patterns, API design guides, refactoring workflows. Always available.' },
            { icon: Globe, title: 'Registry', path: 'Official skill registry', body: 'Browse community-contributed skills. One command install: /skill-install registry:skill-id. Validated before listing.' },
            { icon: FileText, title: 'GitHub', path: 'github.com/user/repo', body: 'Install directly from any public GitHub repository. Must contain a valid SKILL.md with frontmatter. Supports version pinning.' },
          ].map(({ icon: Icon, title, path, body }) => (<article key={title} className="rounded-2xl border border-line bg-card p-7"><Icon className="size-5 text-brand" /><h2 className="mt-8 text-xl font-black text-fg">{title}</h2><code className="mt-2 block font-mono text-xs text-faint">{path}</code><p className="mt-4 text-sm leading-7 text-muted">{body}</p></article>))}
        </div>
      </section>
      ...
      <section className="border-y border-line bg-surface">
        <div className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            ...
            ].map(({ label, body }) => (<div key={label} className="rounded-xl border border-line bg-card p-5"><h3 className="font-black text-sm text-fg">{label}</h3><p className="mt-2 text-xs leading-5 text-muted">{body}</p></div>))}
          </div>
        </div>
      </section>
