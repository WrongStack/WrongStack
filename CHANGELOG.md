...
- **A project daemon that stopped just as it was needed could leave the next start failing for 10 seconds.** A client that finds no Session Catalog or Kanban daemon starts one. If the previous daemon was still releasing the endpoint at that moment, the new one read it as taken and exited, and a moment later nobody held it; the client had started a daemon only once and failed after its whole wait with `connect ENOENT`. It happened when a session opened right as the idle daemon shut down, and it was the long-standing `hq-mailbox-mutation` test flake. Both clients now start another daemon once theirs has exited and no owner is left, as the Mailbox, Chronicle, SAGE and index clients already did.
- **A model availability rule naming the vendor did not cover a second account in the fallback chain.** A rule for `anthropic` blocked a second account `work` (type `anthropic`) while it was running, but not when the fallback chain or `/fallback doctor` checked it by name. Rules now match either the account's config key or the vendor it is built from, everywhere.
